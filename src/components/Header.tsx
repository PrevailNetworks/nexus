...
import HeaderTimeclock from './timeclock/HeaderTimeclock';
...
export const Header: FC = () => {
  ...
  const isManager = typedUser?.role === Role.MANAGER || isAdmin;
  ...
  const supportNavItems = [
    { path: '/timeclock', label: 'Time Clock', icon: Timer },
    { path: '/time-off', label: 'Time Off', icon: Clock },
    { path: '/help-desk', label: 'Help Desk', icon: Headphones },
  ];

  // Admin items - only visible to managers and admins
  const adminNavItems = [
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-center py-2">
            ...
            <nav className="flex items-center gap-3">
              ...
              {/* Admin Dropdown - Only visible to managers and admins */}
              {isManager && (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button 
                      variant="ghost" 
                      className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors ${
                        adminNavItems.some(item => location.pathname === item.path)
                          ? 'bg-primary/10 text-primary shadow-sm'
                          : 'text-muted-foreground hover:text-foreground hover:bg-muted/60'
                      }`}
                    >
